...
function initHeroCard() {
  ...
  computeFocusTop();
  window.addEventListener('resize', computeFocusTop);
  window.addEventListener('load', computeFocusTop);
  ...
  function animate(now) {
    ...
    const progress = Math.min(lenisScrollY / 1000, 1);
...
function initTextReveals() {
  // approach-text: no widthOffset, no indent — untouched
